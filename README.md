...
def handle_call({:rejoindre, pid}, _from, state) do
  ...
  nouveaux_clients = [pid | state.clients]
  {:reply, :ok, %{state | clients: nouveaux_clients}}
end
```

#### `handle_call({:quitter, pid})`
```elixir
...
# 4. Exposer le port via cloudflared (dans un autre terminal)
./cloudflared-linux-amd64 tunnel --url tcp://localhost:4040

# 5. Se connecter depuis n'importe quelle machine
telnet quelquechose.trycloudflare.com 7844
```
